...
public class BlockGame extends JPanel implements Runnable, MouseListener,
                                                 MouseMotionListener,
  ...
  public static void main(String[] args) { new BlockGame(); }
  ...
  public BlockGame() {
    ...
    if (playerCount == 1) {
      ...
      JOptionPane.showOptionDialog(
          ...
          new String[] {"OK"}, "OK");
      JOptionPane.showOptionDialog(
          null, this.p2goal.description(), "Player 2 - Goal",
          JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE, null,
          new String[] {"OK"}, "OK");
      p1ScoreLabel.setForeground(GameColors.BLOCK_COLORS[p1TargetColor]);
  ...
  private void updateScores() {
    ...
    p1ScoreLabel.setText("Player 1's " +
                         (p1goal instanceof BlobGoal ? "area" : "perimeter ") +
                         " score: " + this.p1Score);
    p2ScoreLabel.setText("Player 2's " +
                         (p2goal instanceof BlobGoal ? "area" : "perimeter ") +
                         " score: " + this.p2Score);
  }
  ...
  private void makeMove() {
    ...
    if (turnsLeft == 0) { // == 0 is better than <= 0 because it lets them play
                          // on after the game "ends" if they want
      if (playerCount == 1) {
        ...
        if (p1Score == p2Score) {
          ...
          JOptionPane.showMessageDialog(
              null, "Player " + (p1Score > p2Score ? "1" : "2") + " wins!");
        }
      }
    }
  }
  ...
  public void keyPressed(KeyEvent e) {
    ...
    if (keyCode == KeyEvent.VK_UP) {
      selected = board.getSelectedBlock(xClickPos, yClickPos,
                                        Math.max(curLevel - 1, 0));
    } else if (keyCode == KeyEvent.VK_DOWN) {
      selected = board.getSelectedBlock(
          xClickPos, yClickPos, Math.min(curLevel + 1, board.getMaxDepth()));
    }
  }
