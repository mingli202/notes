...
public class TestUtils {
  static McMetro bestMetroBuilder(String tracksString, int seed) {
    ...
    String[] tsArray =
        tracksString.replace(" ", "").replace("[", "").split("]");
    ...
    for (int i = 0; i < tsArray.length; i++) {
      ...
      int cap;
      if (min_cap !=
          Integer.MAX_VALUE) { // One of the buildings has already been created
      ...
      int genGoodness =
          Math.min(Math.min(b1.occupants(), b2.occupants()), cap) / cost;

      // assertTrue(Math.abs(genGoodness - goodness) <= 10);

      // Save track
      Track curTrack = new Track(new TrackID(i), b1.id(), b2.id(), cost, cap);
      tracks[i] = curTrack;
  ...
  static McMetro maxPassengersBuilder(String tracksString) {
    ...
    for (int i = 0; i < tsArray.length; i++) {
      ...
      buildings.computeIfAbsent(b1Id,
                                x -> new Building(b1Id, Integer.MAX_VALUE));
      buildings.computeIfAbsent(b2Id,
                                x -> new Building(b2Id, Integer.MAX_VALUE));
      Building b1 = buildings.get(b1Id);
      Building b2 = buildings.get(b2Id);

      // Save track
      Track curTrack = new Track(new TrackID(i), b1.id(), b2.id(), 1, goalCap);
      tracks[i] = curTrack;
  ...
  static void checkPassengerSearch(String[] passengers, String[] expected,
                                   String firstLetters) {
    McMetro mcMetro = bestMetroBuilder("1,2,1", 1);
    mcMetro.addPassengers(passengers);
    String[] matches =
        mcMetro.searchForPassengers(firstLetters).toArray(new String[0]);
  ...
  static void testHiring(String intervalsString, int hireable) {
    String[] intervals =
        intervalsString.replace(" ", "").replace("[", "").split("]");
